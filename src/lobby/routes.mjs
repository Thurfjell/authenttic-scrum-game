...
function LobbyRoutes(postOffice, gameLogic) {
    ...
    function getLobbies(req, res) {
        ...
        res.write(lobbyListTemplate(lobbies.map((lobby) => ({ lobbyId: lobby.lobbyId, lobbySize: lobby.lobbySize, name: lobby.lobbyName, playerCount: lobby.playingUsersCount }))))
        res.end()
    }
    ...
    function getLobby(req, res) {
        ...
        const story = lobby.stories.find(story => !story.revealedAt)
        if (!story) {
            gameLogic.startLobby(lobby.lobbyId)
        }

        const mainContent = lobby.playingUsersCount === lobby.lobbySize
            ?
            lobbyStoryTemplate({
                as: story.as,
                reason: story.reason,
    ...
    function joinLobby(req, res) {
        ...
        if (gameLogic.getLobbyByUserId(userId)) {
            res.write("TODO: Error: 'you're already in a lobby'; Action:'Reset and go Back'")
            res.end()
            return
        }
    ...
    function createVote(req, res) {
        ...
        req.on("data", (chunk) => {
            ...
            const lobby = gameLogic.getLobbyByUserId(userId)
            gameLogic.voteStory(lobby.lobbyId, storyId, { userId, userName, vote, voteAt: Date.now() })
